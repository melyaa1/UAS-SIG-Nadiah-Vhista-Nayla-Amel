...
// =====================================
const rumahSakit = [
    { nama: "RSUD Cengkareng", lat: -6.143, lng: 106.738 },
    { nama: "RS Hermina Daan Mogot", lat: -6.158, lng: 106.761 },
    { nama: "RS Royal Taruma", lat: -6.171, lng: 106.783 }
];

const klinik = [
    { nama: "Klinik Sehat Medika", lat: -6.180, lng: 106.760 },
    { nama: "Klinik Medika Jaya", lat: -6.165, lng: 106.745 },
    { nama: "Klinik Harapan", lat: -6.154, lng: 106.781 }
];

const apotek = [
    { nama: "Apotek K24", lat: -6.169, lng: 106.772 },
    { nama: "Apotek Kimia Farma", lat: -6.174, lng: 106.754 },
    { nama: "Apotek Guardian", lat: -6.150, lng: 106.770 }
];
...
rumahSakit.forEach(rs => {
    L.marker([rs.lat, rs.lng], { icon: redIcon })
        ...
        .bindPopup(`<b>${rs.nama}</b><br>Kategori : Rumah Sakit`);
});
...
klinik.forEach(item => {
    L.marker([item.lat, item.lng], { icon: blueIcon })
        .addTo(klinikLayer)
        .bindPopup(`<b>${item.nama}</b><br>Kategori : Klinik`);
});
...
apotek.forEach(item => {
    L.marker([item.lat, item.lng], { icon: greenIcon })
        .addTo(apotekLayer)
        .bindPopup(`<b>${item.nama}</b><br>Kategori : Apotek`);
});
...
tombol.forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        // Logika Filter Cepat sesuai tombol di HTML-mu
        if (nama === "Semua") {
            map.addLayer(rumahSakitLayer);
            map.addLayer(klinikLayer);
            map.addLayer(apotekLayer);
        } else if (nama === "Rumah Sakit") {
            map.addLayer(rumahSakitLayer);
            map.removeLayer(klinikLayer);
            map.removeLayer(apotekLayer);
        }
